...
<div class="p-6 max-w-5xl mx-auto space-y-6"
    ...
    <div class="flex items-center justify-between">
        ...
        @if($lastSynced)
        <span class="text-xs text-gray-400">Last synced: {{ \Carbon\Carbon::parse($lastSynced)->format('d M Y H:i') }}</span>
        @endif
    </div>
    ...
    <div class="bg-blue-50 border border-blue-200 text-blue-700 text-xs rounded-lg px-4 py-3">
        Veriler yerel Meta Insights veritabanından alınmaktadır (aylık harcama toplamları).
        Güncel veri için <a href="{{ route('reports.meta-ads') }}" class="underline font-medium">Meta Ads sayfasından</a> sync yapın.
    </div>

    {{-- Monthly spend table --}}
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
            <h2 class="text-sm font-semibold text-gray-700">Monthly Ad Spend</h2>
            <span class="text-xs text-gray-400">{{ count($months) }} months</span>
        </div>

        @if(empty($months))
        <p class="text-sm text-gray-400 text-center py-10">
            No synced data. Go to <a href="{{ route('reports.meta-ads') }}" class="text-blue-600 underline">Meta Ads</a> and sync first.
        </p>
        @else
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-right px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide">Total Spend</th>
                        <th class="px-4 py-3"></th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-gray-100">
                    @foreach($months as $m)
                    @php $imported = isset($importedIds[$m['id']]); @endphp
                    <tr class="hover:bg-gray-50 {{ $imported ? 'opacity-50' : '' }}">
                        <td class="px-4 py-3 font-medium text-gray-800">{{ \Carbon\Carbon::parse($m['month'] . '-01')->format('F Y') }}</td>
                        <td class="px-4 py-3 text-gray-500 text-xs whitespace-nowrap">
                            {{ $m['date_start'] }} – {{ $m['date_stop'] }}
                        </td>
                        <td class="px-4 py-3 text-right font-mono font-semibold text-gray-800 whitespace-nowrap">
                            {{ number_format($m['amount'], 2) }} {{ $currency }}
                        </td>
        ...
        <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md p-6" @click.stop>
            ...
            <form method="POST" action="{{ route('finance.meta-billing.import') }}" class="space-y-4">
                ...
                <div class="bg-gray-50 rounded-lg px-4 py-3 text-sm">
                    <p class="text-gray-500 text-xs mb-1">Period</p>
                ...
                <div>
                    ...
                    <select name="source_account_id"
                        ...
                        <option value="{{ $acc->id }}">
                            {{ $acc->name }} ({{ $acc->currency }})@if($acc->card_last6) ···{{ $acc->card_last6 }}@endif
                        </option>
                        @endforeach
                    </select>
                </div>
