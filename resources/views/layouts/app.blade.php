...
<div class="flex h-screen overflow-hidden">
    ...
    {{-- Sidebar --}}
    <aside :class="sidebar ? 'w-64' : 'w-16'"
           class="bg-slate-900 flex-shrink-0 flex flex-col transition-all duration-200 ease-in-out overflow-hidden">

        {{-- Brand --}}
        <div class="h-16 flex items-center border-b border-slate-700/60 flex-shrink-0"
             :class="sidebar ? 'px-6' : 'justify-center px-0'">
            <span x-show="sidebar" class="text-white font-bold text-lg tracking-tight whitespace-nowrap">M2H Management</span>
            <span x-show="!sidebar" class="text-white font-bold text-lg">M</span>
        </div>
        ...
        <nav class="flex-1 px-2 py-5 space-y-0.5 overflow-y-auto overflow-x-hidden">
            ...
            <a href="{{ route('dashboard') }}"
               :class="sidebar ? 'px-3 gap-3' : 'justify-center px-0'"
            ...
            <a href="{{ route('leads.index') }}"
               ...
               :title="!sidebar ? 'Leads' : ''">
                ...
                <span x-show="sidebar" class="whitespace-nowrap">Leads</span>
            </a>

            <a href="#"
               :class="sidebar ? 'px-3 gap-3' : 'justify-center px-0'"
               class="group flex items-center py-2 rounded-lg text-sm font-medium text-slate-500 cursor-not-allowed"
               :title="!sidebar ? 'Pipeline (soon)' : ''">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6ZM3.75 15.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25ZM13.5 6a2.25 2.25 0 0 1 2.25-2.25H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6ZM13.5 15.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18v-2.25Z" />
                </svg>
                <span x-show="sidebar" class="whitespace-nowrap">Pipeline</span>
                <span x-show="sidebar" class="ml-auto text-xs text-slate-600 whitespace-nowrap">Soon</span>
            </a>

            <a href="#"
               :class="sidebar ? 'px-3 gap-3' : 'justify-center px-0'"
               class="group flex items-center py-2 rounded-lg text-sm font-medium text-slate-500 cursor-not-allowed"
               :title="!sidebar ? 'Tasks (soon)' : ''">
                <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                </svg>
                <span x-show="sidebar" class="whitespace-nowrap">Tasks</span>
                <span x-show="sidebar" class="ml-auto text-xs text-slate-600 whitespace-nowrap">Soon</span>
            </a>
            ...
            <div class="pt-4 mt-4 border-t border-slate-700/60">
                <a href="{{ route('settings.pipelines.index') }}"
                   ...
                   :title="!sidebar ? 'Settings' : ''">
                    ...
                    <span x-show="sidebar" class="whitespace-nowrap">Settings</span>
                </a>
            </div>
            @endif
        ...
        <div class="border-t border-slate-700/60 flex-shrink-0"
             :class="sidebar ? 'p-4' : 'p-2'">
            <div class="flex items-center"
                 :class="sidebar ? 'gap-3' : 'justify-center'">
                ...
                <div class="flex-1 min-w-0" x-show="sidebar">
                    <p class="text-sm font-medium text-white truncate">{{ auth()->user()->name }}</p>
                    <p class="text-xs text-slate-400 truncate">{{ auth()->user()->roleLabel() }}</p>
                </div>
                <form method="POST" action="{{ route('logout') }}" x-show="sidebar">
                    @csrf
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <header class="h-16 bg-white border-b border-gray-200 flex items-center gap-3 px-4 flex-shrink-0">
            ...
            <h1 class="text-lg font-semibold text-gray-800 flex-1">@yield('heading', 'Dashboard')</h1>
