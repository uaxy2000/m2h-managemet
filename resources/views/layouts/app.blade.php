        ...
        <nav class="flex-1 px-2 py-5 space-y-0.5 overflow-y-auto overflow-x-hidden">
            ...
            <div class="pt-4 mt-4 border-t border-slate-700/60 space-y-0.5">
                <a href="{{ route('reports.index') }}"
                   :class="(sidebar || mobileNav) ? 'px-3 gap-3' : 'lg:justify-center lg:px-0 px-3 gap-3'"
                   class="group flex items-center py-2 rounded-lg text-sm font-medium transition-colors
                          {{ request()->routeIs('reports.index') ? 'bg-slate-700 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}"
                   :title="(!sidebar && !mobileNav) ? 'Reports' : ''">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <a href="{{ route('reports.daily') }}"
                   :class="(sidebar || mobileNav) ? 'px-3 gap-3' : 'lg:justify-center lg:px-0 px-3 gap-3'"
                   class="group flex items-center py-2 rounded-lg text-sm font-medium transition-colors
                          {{ request()->routeIs('reports.daily') ? 'bg-slate-700 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}"
                   :title="(!sidebar && !mobileNav) ? 'Daily Report' : ''">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                ...
                <a href="{{ route('reports.meta-ads') }}"
                   :class="(sidebar || mobileNav) ? 'px-3 gap-3' : 'lg:justify-center lg:px-0 px-3 gap-3'"
                   class="group flex items-center py-2 rounded-lg text-sm font-medium transition-colors
                          {{ request()->routeIs('reports.meta-ads') ? 'bg-slate-700 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}"
                   :title="(!sidebar && !mobileNav) ? 'Meta Ads' : ''">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 0 1 3 19.875v-6.75ZM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V8.625ZM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 0 1-1.125-1.125V4.125Z"/>
                    </svg>
                    <span x-show="sidebar || mobileNav" class="whitespace-nowrap">Meta Ads</span>
                </a>
                ...
                <a href="{{ route('payments.index') }}"
                   :class="(sidebar || mobileNav) ? 'px-3 gap-3' : 'lg:justify-center lg:px-0 px-3 gap-3'"
                   class="group flex items-center py-2 rounded-lg text-sm font-medium transition-colors
                          {{ request()->is('payments*') ? 'bg-slate-700 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }}"
                   :title="(!sidebar && !mobileNav) ? 'Payments' : ''">
                    <svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
        ...
        <header class="h-16 bg-white border-b border-gray-200 flex items-center gap-3 px-4 flex-shrink-0">
            ...
            <div class="flex items-center gap-3 flex-shrink-0">
                ...
                <div x-data="{
                    ...
                    {{-- Badge: standard corner positioning with z-index --}}
                    <span x-show="unread > 0"
                          x-text="unread > 99 ? '99+' : unread"
                          class="absolute top-0 right-0 translate-x-1/2 -translate-y-1/2 z-10 text-[10px] font-bold bg-red-500 text-white rounded-full min-w-[1.25rem] h-5 flex items-center justify-center px-1 leading-none tabular-nums pointer-events-none"></span>
