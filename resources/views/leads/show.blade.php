            ...
            <div class="flex flex-col sm:flex-row sm:items-start sm:gap-4 sm:justify-between gap-2">
                ...
                <div class="sm:flex-shrink-0 sm:text-right sm:max-w-xs">
                    <div class="flex flex-wrap gap-1.5 sm:justify-end mb-2">
                        ...
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-medium whitespace-nowrap inline-flex items-center gap-1"
                              style="background: {{ $tag->color }}; color: {{ $flag ? '#111827' : '#ffffff' }}">@if($flag)<span class="fi fi-{{ $flag }}" style="border-radius:2px"></span>@endif{{ $tag->name }}</span>
                        @empty
