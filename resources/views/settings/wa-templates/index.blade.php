...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    @php
    $allEmpty = $activeTemplates->isEmpty() && $inactiveTemplates->isEmpty();
    @endphp
    ...
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden" x-data="{ open: false }">
        <div class="flex items-center gap-4 px-5 py-4 cursor-pointer hover:bg-gray-50 transition-colors" @click="open = !open">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                    <span class="text-sm font-semibold text-gray-800">{{ $tpl->display_name ?? $tpl->name }}</span>
        ...
        <div x-show="open" x-cloak class="border-t border-gray-100 px-5 py-5 space-y-5">
            ...
            <form method="POST" action="{{ route('settings.wa-templates.update', $tpl) }}" class="space-y-4">
                ...
                <div class="space-y-2">
                    ...
                    @php $mapped = collect($fields)->firstWhere(fn($f) => $f['component'] === 'body' && (int)$f['index'] === $i); @endphp
    ...
    {{-- Divider --}}
    @if($activeTemplates->isNotEmpty() && $inactiveTemplates->isNotEmpty())
    <div class="flex items-center gap-3">
        <div class="flex-1 border-t border-gray-200"></div>
        <span class="text-xs text-gray-400 font-medium uppercase tracking-wide">Pasif Şablonlar</span>
        <div class="flex-1 border-t border-gray-200"></div>
    </div>
    @endif

    {{-- Inactive templates --}}
    @foreach($inactiveTemplates as $tpl)
    @php
        $bodyText  = $tpl->bodyText();
        $varCount  = $tpl->bodyVariableCount();
        $hdrType   = $tpl->headerType();
        ...
        $available = \App\Models\WaTemplate::availableFields();
    @endphp
    <div class="bg-white rounded-xl border border-gray-200 overflow-hidden opacity-60" x-data="{ open: false }">
        <div class="flex items-center gap-4 px-5 py-4 cursor-pointer hover:bg-gray-50 transition-colors" @click="open = !open">
            <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2 flex-wrap">
                    <span class="text-sm font-semibold text-gray-800">{{ $tpl->display_name ?? $tpl->name }}</span>
