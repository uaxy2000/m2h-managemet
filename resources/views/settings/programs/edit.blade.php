...
<div class="bg-white rounded-xl border border-gray-200 p-6 max-w-lg">
    <h2 class="text-sm font-semibold text-gray-700 mb-6">Edit Program</h2>

    <form method="POST" action="{{ route('settings.programs.update', $program) }}">
        @csrf @method('PUT')

        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Country <span class="text-red-500">*</span></label>
                <input type="text" name="country" value="{{ old('country', $program->country) }}" required autofocus
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="number" name="min_investment"
                       value="{{ old('min_investment', $program->min_investment) }}" min="0" step="1000"
                       class="block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:ring-indigo-500 focus:border-indigo-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Currency</label>
                <input type="text" name="currency" value="{{ old('currency', $program->currency) }}"
        ...
        <div class="mb-4">
            ...
            <textarea name="description" rows="3"
                      class="block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:ring-indigo-500 focus:border-indigo-500 resize-none">{{ old('description', $program->description) }}</textarea>
        </div>
